...
public class BillingService {
    ...
    public BillingService(SubscriptionRepository subscriptionRepository, OrganizationService organizationService) {
    ...
    public SubscriptionDto getSubscription(UUID organizationId, UUID userId) {
        ...
        Subscription subscription = subscriptionRepository.findByOrganizationId(organizationId)
                .orElseThrow(() -> new ApiException(HttpStatus.NOT_FOUND, "SUB_NOT_FOUND", "Subscription not found"));

        return new SubscriptionDto(subscription.getPlanType(), subscription.getStatus(), subscription.getCurrentPeriodEnd());
    }

    @Transactional
    public CheckoutSessionResponse createCheckoutSession(UUID organizationId, PlanType planType, UUID userId) {
        ...
        if (member.getRole() != OrganizationRole.OWNER) {
            throw new ApiException(HttpStatus.FORBIDDEN, "ACCESS_DENIED", "Only organization owners can manage billing");
        }

        if (planType == PlanType.FREE) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "INVALID_PLAN", "Cannot checkout for free plan");
        }
        ...
        try {
            ...
            throw new ApiException(HttpStatus.INTERNAL_SERVER_ERROR, "STRIPE_ERROR", "Failed to create checkout session");
        }
    }
}
