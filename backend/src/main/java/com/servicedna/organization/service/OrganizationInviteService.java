...
public class OrganizationInviteService {
  ...
  public InviteDto createInvite(UUID organizationId, CreateInviteRequest request, UUID userId) {
    OrganizationMember inviterMember =
        organizationService.validateUserAccess(organizationId, userId);

    if (inviterMember.getRole() == OrganizationRole.VIEWER
        || inviterMember.getRole() == OrganizationRole.MEMBER) {
      throw new ApiException(
          HttpStatus.FORBIDDEN, "ACCESS_DENIED", "You do not have permission to invite members");
    }

    if (inviteRepository.existsByOrganizationIdAndEmailAndStatus(
        organizationId, request.email(), InviteStatus.PENDING)) {
      throw new ApiException(
          HttpStatus.CONFLICT, "INVITE_EXISTS", "A pending invite for this email already exists");
    }
  ...
  public void acceptInvite(String token, UUID userId) {
    OrganizationInvite invite =
        inviteRepository
            ...
            .orElseThrow(
                () ->
                    new ApiException(
                        HttpStatus.NOT_FOUND, "INVITE_NOT_FOUND", "Invite not found or invalid"));

    if (invite.getStatus() != InviteStatus.PENDING) {
      throw new ApiException(
          HttpStatus.BAD_REQUEST, "INVALID_INVITE", "Invite is no longer pending");
    }
    ...
    if (alreadyMember) {
      throw new ApiException(
          HttpStatus.CONFLICT, "ALREADY_MEMBER", "You are already a member of this organization");
    }

    OrganizationMember newMember =
        new OrganizationMember(UUID.randomUUID(), invite.getOrganization(), user, invite.getRole());
