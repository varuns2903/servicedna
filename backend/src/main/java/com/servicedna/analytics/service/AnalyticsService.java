...
public class AnalyticsService {
  ...
  public SlaReportDto generateSlaReport(UUID organizationId, UUID userId, int days) {
    ...
    List<Incident> recentIncidents =
        incidentRepository.findByOrganizationIdOrderByCreatedAtDesc(organizationId).stream()
    ...
    for (Service service : services) {
      List<Incident> serviceIncidents =
          recentIncidents.stream().filter(i -> i.getAffectedServices().contains(service)).toList();
      ...
      for (Incident incident : serviceIncidents) {
        OffsetDateTime end =
            incident.getResolvedAt() != null ? incident.getResolvedAt() : periodEnd;
      ...
      if (totalPeriodMinutes > 0) {
        uptimePercentage =
            Math.max(0, 100.0 * (totalPeriodMinutes - serviceDowntimeMinutes) / totalPeriodMinutes);
      }
    ...
    if (totalPeriodMinutes > 0 && !services.isEmpty()) {
      ...
      overallUptimePercentage =
          Math.max(
              0, 100.0 * (maxPossibleDowntime - orgTotalDowntimeMinutes) / maxPossibleDowntime);
    }

    double orgMttrMinutes =
        orgResolvedIncidents > 0 ? (double) orgTotalMttrMinutes / orgResolvedIncidents : 0.0;
    long orgUptimeMinutes = (totalPeriodMinutes * services.size()) - orgTotalDowntimeMinutes;
    double orgMtbfHours =
        recentIncidents.size() > 0
            ? (double) orgUptimeMinutes / 60.0 / recentIncidents.size()
            : (double) (totalPeriodMinutes * services.size()) / 60.0;
