...
public class DashboardService {
    ...
    public DashboardSummaryDto getDashboardSummaryInternal(UUID organizationId) {
        ...
        int healthyServices = (int) services.stream()
                .filter(s -> s.getStatus() == ServiceStatus.HEALTHY)
                .count();

        List<Incident> allIncidents = incidentRepository.findByOrganizationIdOrderByCreatedAtDesc(organizationId);
        List<Incident> activeIncidents = allIncidents.stream()
                .filter(i -> i.getStatus() != IncidentStatus.RESOLVED)
                .toList();

        List<IncidentDto> activeIncidentDtos = activeIncidents.stream()
                .map(this::mapIncidentToDto)
                .collect(Collectors.toList());

        List<DashboardServiceOverviewDto> servicesOverview = services.stream()
                .map(this::mapServiceToOverview)
                .collect(Collectors.toList());
    ...
    private DashboardServiceOverviewDto mapServiceToOverview(Service service) {
        ...
        // In a real prod app with millions of pings, this should be an aggregate SQL query or Redis cached
        List<ServicePing> recentPings = servicePingRepository.findByServiceIdOrderByCreatedAtDesc(service.getId())
