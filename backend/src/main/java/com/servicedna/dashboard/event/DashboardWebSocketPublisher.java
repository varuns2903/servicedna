...
public class DashboardWebSocketPublisher {
    ...
    public DashboardWebSocketPublisher(DashboardService dashboardService, SimpMessagingTemplate messagingTemplate) {
    ...
    public void handleDashboardInvalidation(DashboardInvalidationEvent event) {
        log.info("Invalidating dashboard cache and broadcasting update for Org: {}", event.getOrganizationId());
        ...
        DashboardSummaryDto newSummary = dashboardService.getDashboardSummaryInternal(event.getOrganizationId());
