...
public class AuthService {
    ...
    public AuthResponse register(RegisterRequest request) {
        if (userRepository.existsByEmail(request.email())) {
            throw new ApiException(HttpStatus.CONFLICT, "USER_EXISTS", "User already exists with this email.");
        }
        ...
        User user = new User(
                ...
                Role.OWNER // Default role for now, Phase 3 will introduce Organizations and better Role assignment
        );
        ...
        return new AuthResponse(jwtToken, new UserDto(user.getId(), user.getEmail(), user.getRole().name()));
    }
