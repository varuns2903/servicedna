...
public class PublicStatusService {
  ...
  public PublicStatusPageDto getPublicStatusPage(UUID organizationId) {
    ...
    List<com.servicedna.service.domain.Service> services =
        serviceRepository.findByOrganizationId(organizationId);
    List<PublicServiceDto> serviceDtos =
        services.stream()
            .map(
                s ->
                    new PublicServiceDto(s.getId(), s.getName(), s.getDescription(), s.getStatus()))
            .collect(Collectors.toList());

    List<com.servicedna.incident.domain.Incident> incidents =
        incidentRepository.findByOrganizationIdOrderByCreatedAtDesc(organizationId);
    List<PublicIncidentDto> activeIncidentDtos =
        incidents.stream()
