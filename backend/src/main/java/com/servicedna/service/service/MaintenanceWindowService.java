...
public class MaintenanceWindowService {
    ...
    @Transactional
    public MaintenanceWindowDto createMaintenanceWindow(UUID organizationId, CreateMaintenanceWindowRequest request, UUID userId) {
        ...
        Service service = serviceRepository.findByOrganizationIdAndId(organizationId, request.serviceId())
                .orElseThrow(() -> new ApiException(HttpStatus.NOT_FOUND, "SERVICE_NOT_FOUND", "Service not found"));

        if (request.startTime().isAfter(request.endTime())) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "INVALID_TIME", "Start time must be before end time");
        }
    ...
    public List<MaintenanceWindowDto> getMaintenanceWindows(UUID organizationId, UUID userId) {
        organizationService.validateUserAccess(organizationId, userId);
        return maintenanceWindowRepository.findByOrganizationIdOrderByStartTimeDesc(organizationId).stream()
                .map(this::mapToDto)
                .collect(Collectors.toList());
    }

    @Transactional
    public MaintenanceWindowDto updateMaintenanceStatus(UUID organizationId, UUID windowId, MaintenanceStatus status, UUID userId) {
        organizationService.validateUserAccess(organizationId, userId);

        MaintenanceWindow window = maintenanceWindowRepository.findByOrganizationIdAndId(organizationId, windowId)
                .orElseThrow(() -> new ApiException(HttpStatus.NOT_FOUND, "WINDOW_NOT_FOUND", "Maintenance window not found"));
