...
public interface MaintenanceWindowRepository extends JpaRepository<MaintenanceWindow, UUID> {
    ...
    @Query("SELECT COUNT(m) > 0 FROM MaintenanceWindow m WHERE m.service.id = :serviceId " +
           "AND m.status = 'IN_PROGRESS' " +
           "AND m.startTime <= :now AND m.endTime >= :now")
    boolean isServiceInActiveMaintenance(@Param("serviceId") UUID serviceId, @Param("now") OffsetDateTime now);
}
