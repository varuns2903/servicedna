...
public class AlertEventPublisher {
  ...
  public AlertEventPublisher(
    ...
    // ensure JavaTimeModule is registered (usually handled by Spring Boot's autoconfigured
    // ObjectMapper, but explicitly ensuring it here is safe)
  ...
  public void publishStatusChangedEvent(ServiceStatusChangedEvent event) {
    try {
      ...
      kafkaTemplate.send(
          KafkaTopicConfig.SERVICE_EVENTS_TOPIC, event.serviceId().toString(), payload);
