...
public class AlertRuleService {
    ...
    @Transactional
    public AlertRuleDto createAlertRule(UUID organizationId, UUID serviceId, CreateAlertRuleRequest request, UUID userId) {
    ...
    public void deleteAlertRule(UUID organizationId, UUID ruleId, UUID userId) {
        organizationService.validateUserAccess(organizationId, userId);
        AlertRule rule = alertRuleRepository.findByOrganizationIdAndId(organizationId, ruleId)
                .orElseThrow(() -> new ApiException(HttpStatus.NOT_FOUND, "RULE_NOT_FOUND", "Alert rule not found"));
