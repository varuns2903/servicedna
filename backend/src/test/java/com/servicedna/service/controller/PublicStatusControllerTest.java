...
class PublicStatusControllerTest {
    ...
    void setUp() throws Exception {
        ...
        String userToken = objectMapper.readTree(registerRes).get("token").asText();
        ...
        mockMvc.perform(post("/api/v1/organizations/" + orgId + "/services")
                .header("Authorization", "Bearer " + userToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(new CreateServiceRequest("Web App", "Frontend", "http://repo", "us-east-1", null))))
                .andExpect(status().isCreated());
    }
