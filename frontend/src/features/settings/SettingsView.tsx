...
import { Building2, Users, CreditCard, Send, Check, X, UserCog, Phone, FileText, AlertTriangle, Download, Webhook, Gauge } from 'lucide-react';
...
export function SettingsView() {
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'members' | 'billing' | 'account' | 'oncall' | 'escalation' | 'integrations' | 'sla' | 'audit'>('general');
    ...
    <div className="p-4 space-y-6 md:p-8">
      ...
      <div className="flex space-x-4 overflow-x-auto border-b border-charcoal-700 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        <button
          ...
          className={`shrink-0 pb-3 text-sm font-medium transition-colors border-b-2 ${
            activeTab === 'sla' ? 'border-emerald-500 text-white' : 'border-transparent text-gray-400 hover:text-gray-300'
          }`}
        >
          <div className="flex items-center space-x-2">
      ...
      <div className="pt-4">
        ...
        {activeTab === 'sla' && currentOrgId && <SlaReportTab orgId={currentOrgId} />}
